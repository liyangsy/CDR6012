...
// allen@20150827, load properties from all ini files in path
static void _oem_read_properties_from_ini_file(const char * path) {
	struct dirent *de;
	char tmp_path[256];
	DIR *dir;
	...
	while ((de = readdir(dir)) != 0) {
	    if (!strcmp(de->d_name, ".") || !strcmp(de->d_name, "..") || !strstr(de->d_name,".ini"))
	       continue;
	    snprintf(tmp_path, sizeof(tmp_path), "%s/%s", path, de->d_name);
	    //printf("_oem_read_prorperties_from_ini_file %s\n", tmp_path);
	    load_properties_from_file(tmp_path, NULL);
	}
...
void load_system_props() {
    _oem_read_properties_from_ini_file("/persist");
