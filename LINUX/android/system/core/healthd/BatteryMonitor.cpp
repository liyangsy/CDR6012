...
bool BatteryMonitor::update(void) {
    ...
    props.batteryAging = getIntField(mHealthdConfig->batteryBatteryAgingPath);
    props.batteryAcc = getIntField(mHealthdConfig->batteryBatteryAccPath);
    ...
    if (dir == NULL) {
        ...
        while ((entry = readdir(dir))) {
			...
			if (access(path.string(), R_OK) == 0) {
				props.chargerUsbOnline = getBooleanField(path);
				//switch(getIntField(path)){
				//		case 0:
			//				props.chargerUsbOnline = false;
				//		case 1:
				//		    props.chargerUsbOnline = true;
				//}
				//props.chargerUsbOnline = getIntField(path);
			}
			/*
...
void BatteryMonitor::init(struct healthd_config *hc) {
    ...
    if (dir == NULL) {
        ...
        while ((entry = readdir(dir))) {
            ...
            case ANDROID_POWER_SUPPLY_TYPE_BATTERY:
                ...
                if (mHealthdConfig->batteryBatteryAgingPath.isEmpty()) {
                    path.clear();
                    path.appendFormat("%s/bms/cycle_count",
                                      POWER_SUPPLY_SYSFS_PATH);
                    if (access(path, R_OK) == 0)
                        mHealthdConfig->batteryBatteryAgingPath = path;
                }

                if (mHealthdConfig->batteryBatteryAccPath.isEmpty()) {
                    path.clear();
                    path.appendFormat("%s/battery/acc_status",
                                      POWER_SUPPLY_SYSFS_PATH);
                    if (access(path, R_OK) == 0)
                        mHealthdConfig->batteryBatteryAccPath = path;
                }
    ...
    if (!mBatteryDevicePresent) {
        ...
        if (mHealthdConfig->batteryBatteryAgingPath.isEmpty())
            KLOG_WARNING(LOG_TAG, "BatteryAgingPath not found\n");    
        if (mHealthdConfig->batteryBatteryAccPath.isEmpty())
            KLOG_WARNING(LOG_TAG, "BatteryAccPath not found\n");        
    }
