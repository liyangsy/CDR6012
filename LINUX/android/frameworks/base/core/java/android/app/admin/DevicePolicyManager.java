...
/**
     * @param sec_time which set time in second
      *
     */
    public void setBootAnimInterval(int sec_time) {
        try {
            mService.setBootAnimInterval(sec_time);
        } catch (RemoteException re) {
            Log.w(TAG, "Failed set boot animation time", re);
        }
    }

    /**
     * Returns the current value  set by the device or profile owner
     * @return the current value for boot animation
     */
    public int getBootAnimInterval() {
        try {
            return mService.getBootAnimInterval();
        } catch (RemoteException re) {
            return 0;
        }
    }
