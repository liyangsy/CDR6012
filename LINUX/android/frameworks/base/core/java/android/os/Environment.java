...
public class Environment {
    ...
    /** {@hide} */
    public static final String DIR_ANDROID = "System";
    ...
    /** {@hide} */
    public static final String DIRECTORY_RECORDER = "_recorder";
    /** {@hide} */
    public static final String DIRECTORY_LOG = "_log";

    public static File getRecorderDirectory() {
        return new File(getExternalStorageDirectory(), DIRECTORY_RECORDER);
    }

    public static File getLogDirectory() {
        return new File(getExternalStorageDirectory(), DIRECTORY_LOG);
    }

    public static int getRecorderFreeSpaceMb() {
        int errorVal = -1;
        if (!getExternalStorageState().equals(MEDIA_MOUNTED)) {
            Log.i(TAG, "SD card is not mounted");
            return errorVal;
        }
        final int maxMb = SystemProperties.getInt(StorageManager.PROP_SDCARD_RECORDER_MAX_MB, -1);
        if (-1 == maxMb) return errorVal;

        final long usedKb = getDirUsedSpace(getRecorderDirectory());
        if (usedKb < 0) return errorVal;

        return maxMb - (int)(usedKb / 1024);
    }

    public static int getLogFreeSpaceMb() {
        int errorVal = -1;
        if (!getExternalStorageState().equals(MEDIA_MOUNTED)) {
            Log.i(TAG, "SD card is not mounted");
            return errorVal;
        }
        final int maxMb = SystemProperties.getInt(StorageManager.PROP_SDCARD_LOG_MAX_MB, -1);
        if (-1 == maxMb) return errorVal;

        final long usedKb = getDirUsedSpace(getLogDirectory());
        if (usedKb < 0) return errorVal;
     ...
     * @hide
     */
    public static long getDirUsedSpace(File dir) {
        BufferedReader reader = null;
        try {
            // get used space
            java.lang.Process process = Runtime.getRuntime().exec("/system/bin/du -s -k " +
            ...
            String[] result = output.toString().split(" |\t", 2);
            //Log.d("allen", "getDirUsedSpace = " + Long.parseLong(result[0]) + " KB");
