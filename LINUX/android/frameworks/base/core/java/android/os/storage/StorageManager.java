...
public class StorageManager {
    ...
    // allen@20161117, Add SD card space related APIs
    /** @hide */
    public static final String PROP_SDCARD_RECORDER_USE_RATIO = "persist.sys.sd.rec_use_ratio";
    /** @hide */
    public static final String PROP_SDCARD_TOTAL_MB = "persist.sys.sd.total_mb";
    /** @hide */
    public static final String PROP_SDCARD_RECORDER_MAX_MB = "persist.sys.sd.recorder_max_mb";
    /** @hide */
    public static final String PROP_SDCARD_LOG_MAX_MB = "persist.sys.sd.log_max_mb";

    public static boolean setSdcardRecorderUseRatio(int ratio) {
        if (ratio < 0 || ratio > 100) {
            throw new IllegalArgumentException("ratio value must be 0 ~ 100");
        }
        ...
        final long totalBytes = sdcard.getTotalSpace();
        if (0L == totalBytes) return false;
        final int totalMb = (int)(totalBytes / (1024 * 1024));
        //Log.d("allen", "totalBytes="+totalBytes+", totalMb="+totalMb);

        final int recorderMaxMb = totalMb * ratio / 100;
        final int logMaxMb = totalMb * (100 - ratio) / 100;
        SystemProperties.set(PROP_SDCARD_RECORDER_MAX_MB, String.valueOf(recorderMaxMb));
        SystemProperties.set(PROP_SDCARD_LOG_MAX_MB, String.valueOf(logMaxMb));
        SystemProperties.set(PROP_SDCARD_RECORDER_USE_RATIO, String.valueOf(ratio));
        return true;
    }

    public static int getSdcardRecorderUseRatio() {
        return SystemProperties.getInt(StorageManager.PROP_SDCARD_RECORDER_USE_RATIO, -1);
    }
}
