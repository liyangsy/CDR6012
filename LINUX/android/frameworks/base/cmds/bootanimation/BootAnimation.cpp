...
#define INTERVAL_PROP_NAME "persist.sys.banim.time"
//#define INTERVAL_PROP_NAME "service.bootanim.interval"
...
bool BootAnimation::movie()
{
    char value[PROPERTY_VALUE_MAX];
    String8 desString;
    char times[PROPERTY_VALUE_MAX];
    property_get(INTERVAL_PROP_NAME,times,"0");
    int timeinterval= atoi (times);
    ALOGD("Movie Bootanim interval : %d",   timeinterval);
    nsecs_t startMovieTime = systemTime();
    ...
    for (size_t i=0 ; i<pcount ; i++) {
        ...
        for (int r=0 ; !part.count || r<part.count ; r++) {
            ...
            for (size_t j=0 ; j<fcount && (!exitPending() || part.playUntilComplete) ; j++) {
                ...
                nsecs_t AnimInterval= now -  startMovieTime;
                AnimInterval = nanoseconds_to_seconds(AnimInterval); 
                
                if( AnimInterval > timeinterval )
                    checkExit();
            }
