...
public class WifiManager {
    ...
    //@hide
    public static final int AP_WPS_STATUS_INACTIVE = 0;
    //@hide
    public static final int AP_WPS_STATUS_PROCESSING = 1;
    //@hide
    public static final int AP_WPS_STATUS_COMPLETED = 2;
    //@hide
    public static final int AP_WPS_STATUS_FAILED_TIMEOUT = 3;
    //@hide
    public static final int AP_WPS_STATUS_FAILED_OVERLAP = 4;
    ...
    */
     ...
     * @hide Dont open yet
     */
    public int getApWpsStatus() {
        try {
            return mService.getApWpsStatus();
        } catch (RemoteException e) {
            return AP_WPS_STATUS_INACTIVE;
        }
    }
   ...
   /**
     ...
     * @internal
     */
    public boolean startApWpsPbc() {
        try {
            return mService.startApWpsPbc();
        } catch (RemoteException e) {
            return false;
        }
    }
    ...
    /**
     * AP WPS PIN code for keypad mode
     * @param pinCode for 4 digit or 7+1 digit
     * @return {@code true} if the operation succeeds else {@code false}
     * @hide
     * @internal
     */
    public boolean startApWpsPin(String pinCode) {
        try {
            return mService.startApWpsPin(pinCode);
        } catch (RemoteException e) {
            return false;
        }
    }
    ...
    /**
     * AP WPS Cancel for running
     * @return {@code true} if the operation succeeds else {@code false}
     * @hide
     * @internal
     */
    public boolean cancelApWps() {
        try {
            return mService.cancelApWps();
        } catch (RemoteException e) {
            return false;
        }
    }
    ...
    /**
     * Get wifi profile from SD
     * @return {@code true} if the operation succeeds else {@code false}
     * @hide
     * @internal
     */
    public int setWifiEnableFromSD() {
        WifiProfile mWifiProfile = null;
        mWifiProfile = new WifiProfile(mContext);

        if (mWifiProfile.failMsg != 0)
            return mWifiProfile.failMsg;

        if(!isWifiEnabled()) {
            Log.d(TAG, "WifiDisable, Enable Wifi...");
            setWifiEnabled(true);
            try{
                Thread.sleep(2000);
        ...
        //Check SSID Exists
        int networkId = checkIfSSIDExists(mWifiProfile);
        if (networkId == -1) {
            networkId = addWifiConfiguration(networkId, mWifiProfile);
        }
        if (networkId == -1) {
            Log.d(TAG, "Invalid wifi network");
        }
        final int final_networkId = networkId;
        Log.d(TAG, "Wifi networkID:" + final_networkId);
        return final_networkId;
    }

    private int checkIfSSIDExists(WifiProfile mWifiProfile) {
        for(WifiConfiguration i : getConfiguredNetworks())
        {
            if(i.SSID != null && i.SSID.equals("\"".concat(mWifiProfile.ssid).concat("\"")))
            {
    ...
    private int addWifiConfiguration(int networkId, WifiProfile mWifiProfile) {
        WifiConfiguration wfc = new WifiConfiguration();
        wfc.SSID = "\"".concat(mWifiProfile.ssid).concat("\"");
        ...
        if (mWifiProfile.security == WifiProfile.OPEN) { // no password
            wfc.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.NONE);
            wfc.allowedAuthAlgorithms.clear();
            wfc.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.CCMP);
            wfc.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.TKIP);
        } else if (mWifiProfile.security == WifiProfile.WEP) { // WEP
            wfc.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.NONE);
            wfc.allowedAuthAlgorithms.set(WifiConfiguration.AuthAlgorithm.OPEN);
            wfc.allowedAuthAlgorithms.set(WifiConfiguration.AuthAlgorithm.SHARED);

            // if hex string
            // tudo 20161216 Fix WEP couldn't work in WiFi client mode
            wfc.wepKeys[0] = mWifiProfile.password;
            //wfc.wepKeys[0] = "\"".concat(mWifiProfile.password).concat("\"");
            wfc.wepTxKeyIndex = 0;
        } else if (mWifiProfile.security == WifiProfile.WPA) { // WPA(2)
            wfc.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.WPA_PSK);
            wfc.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.CCMP);
            wfc.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.TKIP);

            wfc.preSharedKey = "\"".concat(mWifiProfile.password).concat("\"");
        }
        int result = addNetwork(wfc);
        if (result  != -1) {
            networkId = result;
        }
        return networkId;
     ...
     * @internal
     */
    public int setWifiApEnableFromSD() {
        String mProfile = "wifiap.xml";
        WifiProfile mWifiProfile = null;
        mWifiProfile = new WifiProfile(mContext, mProfile);

        if (mWifiProfile.failMsg != 0)
            return mWifiProfile.failMsg;

        if (isWifiApEnabled()) {
            Log.d(TAG, "AP active, Disable it...");
        ...
        int result = (addWifiApConfiguration(mWifiProfile)) ? 0 : -1;
        return result;
    }

    private boolean addWifiApConfiguration(WifiProfile mWifiProfile) {
        WifiConfiguration wfc = new WifiConfiguration();
        wfc.SSID = mWifiProfile.ssid;
        if (mWifiProfile.security == WifiProfile.OPEN) {
            wfc.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.NONE);
        } else if (mWifiProfile.security == WifiProfile.WEP) {
            Log.w(TAG,"WEP mode are not support under SoftAP");
            return false;
            //wfc.allowedKeyManagement.set(KeyMgmt.NONE);
        } else if (mWifiProfile.security == WifiProfile.WPA) {
            wfc.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.WPA2_PSK);
            wfc.preSharedKey = mWifiProfile.password;
        }
        return setWifiApEnabled(wfc, true);
    }
