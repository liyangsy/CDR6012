...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        try {
            ...
            if (showNav) {
                ...
                mNavigationBarViewP =
                    (NavigationBarView) View.inflate(context, R.layout.navigation_bar_pioneer, null);

                mNavigationBarViewP.setDisabledFlags(mDisabled1);
                mNavigationBarViewP.setBar(this);
                mNavigationBarViewP.setOnVerticalChangedListener(
                        new NavigationBarView.OnVerticalChangedListener() {
                    @Override
                    public void onVerticalChanged(boolean isVertical) {
                        if (mAssistManager != null) {
                ...
                mNavigationBarViewP.setOnTouchListener(new View.OnTouchListener() {
                    @Override
                    public boolean onTouch(View v, MotionEvent event) {
                        checkUserAutohide(v, event);
                        return false;
                    }});
                mWindowManager.addView(mNavigationBarViewP, getNavigationBarLayoutParamsPioneer());
            }
        ...
        mGps = (ImageView) mStatusBarView.findViewById(R.id.imgGps);
        // Other icons
        ...
        locationfilter.addAction(LocationManager.GPS_ENABLED_CHANGE_ACTION);
        locationfilter.addAction(LocationManager.GPS_FIX_CHANGE_ACTION);
        context.registerReceiverAsUser(mLocationReceiver, UserHandle.ALL, locationfilter, null, null);
    ...
    private WindowManager.LayoutParams getNavigationBarLayoutParamsPioneer() {
        WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
                LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT,
                WindowManager.LayoutParams.TYPE_NAVIGATION_BAR_PIONEER,
                    0
                    | WindowManager.LayoutParams.FLAG_TOUCHABLE_WHEN_WAKING
                    | WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
                    | WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL
        ...
        lp.setTitle("NavigationBarPioneer");
        lp.windowAnimations = 0;
        return lp;
    }
    ...
    @Override // CommandQueue
    public void setNavigationBarItem(int right_1, int right_2, int right_3, int bottom_1) {
        if (mNavigationBarView != null && mNavigationBarViewP != null) {
            mNavigationBarView.SetRecentsIcon(Resource.getRsource(right_1));
            mNavigationBarView.SetHomeIcon(Resource.getRsource(right_2));
            mNavigationBarView.SetBackIcon(Resource.getRsource(right_3));
            mNavigationBarViewP.SetBackIcon(Resource.getRsource(bottom_1));
        }
    }
    ...
    private BroadcastReceiver mLocationReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(LocationManager.GPS_FIX_CHANGE_ACTION) && enabled) {
                ...
                mGps.setImageResource(R.drawable.stat_sys_gps_4);
            } else if (action.equals(LocationManager.GPS_ENABLED_CHANGE_ACTION) && !enabled) {
                // GPS is off
                mGps.setImageResource(R.drawable.stat_sys_gps_0);
            } else {
                // GPS is on, but not receiving fixes
                if(svCount > 0)
                    mGps.setImageResource(R.drawable.stat_sys_gps_1);
                else
                    mGps.setImageResource(R.drawable.stat_sys_gps_0);
            }
        }
