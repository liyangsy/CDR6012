...
public class NavigationBarView extends LinearLayout {
    ...
    private void getIcons(Resources res) {
        //hardcoded for 6010 right navigation bar, originally ic_sysbar_back
        mBackIcon = res.getDrawable(R.drawable.p0139_btguid_icon03_5dis);//
        mBackLandIcon = mBackIcon;
        mBackAltIcon = res.getDrawable(R.drawable.ic_sysbar_back_ime);
        mBackAltLandIcon = mBackAltIcon;
        //hardcoded for 6010 right navigation bar, originally ic_sysbar_recent
        mRecentIcon = res.getDrawable(R.drawable.p0136_btguid_icon02_5dis);
        mRecentLandIcon = mRecentIcon;
    }

    public void SetBackIcon(int resId) {
        KeyButtonView backBnt = (KeyButtonView) getBackButton();
        if (resId == Resource.blank)
            backBnt.setVisibility(View.INVISIBLE);
        else if(resId != Resource.blank && resId != Resource.nochange) {
            backBnt.setVisibility(View.VISIBLE);
            backBnt.setImageResource(resId);
        }
    }

    public void SetHomeIcon(int resId) {
        KeyButtonView homeBnt = (KeyButtonView) getHomeButton();
        if (resId == Resource.blank)
            homeBnt.setVisibility(View.INVISIBLE);
        else if(resId != Resource.blank && resId != Resource.nochange) {
            homeBnt.setVisibility(View.VISIBLE);
            homeBnt.setImageResource(resId);
        }
    }

    public void SetRecentsIcon(int resId) {
        KeyButtonView recentsBnt = (KeyButtonView) getRecentsButton();
        if (resId == Resource.blank)
            recentsBnt.setVisibility(View.INVISIBLE);
        else if(resId != Resource.blank && resId != Resource.nochange) {
            recentsBnt.setVisibility(View.VISIBLE);
            recentsBnt.setImageResource(resId);
        }
    }
