...
public final class PowerManagerService extends SystemService
    ...
    // charge cycle for battery aging
    private static final int AGING_CHECK_INTERVAL = 	2000; //(2sec)
	private static final int BLOCKER_AGING_STARTBYTE =  0;
	private static final int BLOCKER_INIT_VALUE = 0;
	private static final int AGING_STABLE_INCREASED = 1;
    private int mAgingCurrentValue = 0;
	private boolean mAgingReadyState = false;
    ...
    // The suspend blocker used to keep the CPU alive when power in has acquired
    // a wake lock.
    private final SuspendBlocker mWakeLockPowerBlocker;
    ...
    /* for charge cycle */
    private static native void nativeSetChargeCycletoKernel(int value);
    private static native void nativeSetChargeCycletoBlocker(int startbyte,int value);
    private static native int nativeGetChargeCyclefromBlocker(int startbyte);
    ...
    public PowerManagerService(Context context) {
        ...
        synchronized (mLock) {
            ...
            mWakeLockPowerBlocker = createSuspendBlockerLocked("PowerManagerService.Power");
    ...
    public void systemReady(IAppOpsService appOps) {
        synchronized (mLock) {
            ...
            mHandler.postDelayed(mBatteryAgingCheck, AGING_CHECK_INTERVAL);
        }
    }

	 /*  Add timer for battery aging check process  */
    private Runnable mBatteryAgingCheck = new Runnable() {
        public void run() {
            synchronized (mLock) {
				mAgingCurrentValue = nativeGetChargeCyclefromBlocker(BLOCKER_AGING_STARTBYTE);  // get charge cycle

				if( mAgingCurrentValue >= BLOCKER_INIT_VALUE)
					nativeSetChargeCycletoKernel(mAgingCurrentValue);  
				else{
					Slog.w(TAG,"Invalid partition aging value , need be reset to zero.");
					nativeSetChargeCycletoBlocker(BLOCKER_AGING_STARTBYTE,BLOCKER_INIT_VALUE);  
				}

				mAgingReadyState = true;
            }
        }
    };
    ...
    private void updateIsPoweredLocked(int dirty) {
        if ((dirty & DIRTY_BATTERY_STATE) != 0) {
            ...
            if (wasPowered != mIsPowered || oldPlugType != mPlugType) {
				...
				if(mIsPowered)
                    mWakeLockPowerBlocker.acquire();
                else
                    mWakeLockPowerBlocker.release();
    ...
    private int getAccStateInternal() {
        return mBatteryManagerInternal.getAccCurrentState();
    }
    
    private void resetBatteryAgingInternal() {
        synchronized (mLock) {
			mAgingCurrentValue = BLOCKER_INIT_VALUE;
			nativeSetChargeCycletoBlocker(BLOCKER_AGING_STARTBYTE,mAgingCurrentValue);
			nativeSetChargeCycletoKernel(mAgingCurrentValue);
        }
    }
    ...
    private final class BatteryReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
				int aging = intent.getIntExtra(BatteryManager.EXTRA_BATTERY_AGING,BLOCKER_INIT_VALUE);	

				if(mAgingReadyState){
                    if((aging - mAgingCurrentValue) == AGING_STABLE_INCREASED){
					    nativeSetChargeCycletoBlocker(BLOCKER_AGING_STARTBYTE,aging);  // charge cycle
					    mAgingCurrentValue = aging;
					    Slog.i(TAG,"Battery aging save to blocker("+mAgingCurrentValue+")");
					}else if (aging != mAgingCurrentValue){
						nativeSetChargeCycletoKernel(mAgingCurrentValue);  
					}
				}
    ...
    private final class BinderService extends IPowerManager.Stub {
        ...
        @Override // Binder call
        public void resetBatteryAgingState() {
            final long ident = Binder.clearCallingIdentity();
            try {
                resetBatteryAgingInternal();
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
        }
        ...
        public void reboot(boolean confirm, String reason, boolean wait) {
            ...
            }else if (PowerManager.REBOOT_CHARGING_MODE.equals(reason)  || PowerManager.REBOOT_HOT_LIMITED_MODE.equals(reason) ){
				Slog.d(TAG,"Reboot to "+ reason +" mode !! ");
			}else
				mContext.enforceCallingOrSelfPermission(android.Manifest.permission.REBOOT, null);
