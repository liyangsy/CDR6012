...
public class StatusBarManagerService extends IStatusBarService.Stub {
    ...
    @Override
    public void setNavigationBarItem(final int right_1, final int right_2, final int right_3, final int bottom_1) {
        synchronized (mLock) {
            mHandler.post(new Runnable() {
                public void run() {
                    if (mBar != null) {
                        try {
                            mBar.setNavigationBarItem(right_1, right_2, right_3, bottom_1);
                        } catch (RemoteException ex) {
                        }
                    }
                }
