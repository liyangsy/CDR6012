...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static public final String INTENT_ACTION_SET_NAVIGATION_BAR_ITEM = "android.intent.action.set_navigation_bar_item";
    static public final String INTENT_ACTION_KEY = "items";
    public static final int p0136_btguid_icon02_5dis = 136;
    public static final int p0148_btguid_icon06_5dis = 148;
    public static final int p0139_btguid_icon03_5dis = 139;
    public static final int p0178_btguid_icon16_5dis = 178;
    ...
    WindowState mNavigationBarPioneer = null;
    ...
    static final Rect mTmpNavigationFrameP = new Rect();
    ...
    private boolean mScreenshotChordDpadDownKeyTriggered;
    private long mScreenshotChordDpadDownKeyTime;
    private boolean mScreenshotChordDpadDownKeyConsumed;
    // Add by SKYSOFT end
    ...
    /* power key 0~2 2~10 10~* 
     *  interval set to 2000 and 8000*/
    private static final int CDR_POWERKEY_INTERVAL_SHORT = 2000; 
    private static final int CDR_POWERKEY_INTERVAL_LONG = 8000; 
    
    private static final int PKEY_TRIGGER_NORMAL = 0;
    private static final int PKEY_TRIGGER_SHORT = 1;
    private static final int PKEY_TRIGGER_LONG = 2;
    
    private static final int POWER_KEY_MODE_NONE = 0; 
    private static final int POWER_KEY_MODE_SHORT = 1;   // < 2 sec
    private static final int POWER_KEY_MODE_NORMAL = 2;   // 2 < time < 10 sec
    
    private boolean mPowerKeySet;
    private boolean mPowerKeyWakeupSet;
    private int mPowerKeyMode = POWER_KEY_MODE_NONE;
    ...
    private static final int KEY_MODE_UP = 0;
    private static final int KEY_MODE_SHORT_PRESS = 1;   // <= 2 sec
    private static final int KEY_MODE_LONG_PRESS = 2;   // 2 sec < time
    private int mRecordedKey = 0;
    ...
    private static final int MSG_CDR_POWER_LONG_PRESS_SMODE = 16;
    private static final int MSG_CDR_POWER_LONG_PRESS_LMODE = 17;

    boolean mWifiDisplayConnected = false;
    int mWifiDisplayCustomRotation = -1;

    private final static int DEFAULT_INTERVAL_MS = 500;
    private final static int RESET = 0;
    private final static int DOWN = 1;
    private final static int DOWN_UP = 2;
    private final static int DOWN_UP_UP = 3;
    private final static int UP = 4;
    private final static int UP_DOWN = 5;
    private final static int UP_DOWN_DOWN = 6;
    private int mStep = RESET;
    private long mCurrentTime = 0;
    private long mLastTime = 0;
    private long mInterval = 0;
    private int mDevelopStep = RESET;
    private long mDevelopCurrentTime = 0;
    private long mDevelopLastTime = 0;
    private long mDevelopInterval = 0;
    private boolean mBackDown = false;
    private boolean mEnterPressed = false;
    private final boolean mUserBuild = SystemProperties.get("ro.build.type").equals("user");
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_POWER_LONG_PRESS:
                    powerLongPress();
                    break;
                case MSG_CDR_POWER_LONG_PRESS_SMODE:
                    CDRPowerLongPress(POWER_KEY_MODE_NORMAL);
					break;
				case MSG_CDR_POWER_LONG_PRESS_LMODE:
				    CDRPowerLongPress(POWER_KEY_MODE_NONE);
					break; 	    
    ...
    private final BarController mNavigationBarPioneerController = new BarController("NavigationBarPioneer",
            View.NAVIGATION_BAR_TRANSIENT,
            View.NAVIGATION_BAR_UNHIDE,
            View.NAVIGATION_BAR_TRANSLUCENT,
            StatusBarManager.WINDOW_NAVIGATION_BAR,
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        mPowerKeyHandled = hungUp || mScreenshotChordVolumeDownKeyTriggered || mScreenshotChordDpadDownKeyTriggered /* Add by SKYSOFT */
...
//                            ViewConfiguration.get(mContext).getDeviceGlobalActionKeyTimeout());
                    mHandler.sendMessageDelayed(msg, CDR_POWERKEY_INTERVAL_SHORT);  //  2 sec

                }
            } else {
				sendPowerkeyIntent(POWER_KEY_MODE_SHORT);
    ...
    private void CDRPowerLongPress(int NextMode) {
		if(NextMode == POWER_KEY_MODE_NORMAL){
			// set to mode2 and create a delay msg for 8 sec
			mPowerKeyMode = NextMode;
			Message msg = mHandler.obtainMessage(MSG_CDR_POWER_LONG_PRESS_LMODE);
            msg.setAsynchronous(true);
            mHandler.sendMessageDelayed(msg,CDR_POWERKEY_INTERVAL_LONG);
		}else{
			// set to none and broadcast over 10sec Intent
			mPowerKeyMode = NextMode;
			sendPowerkeyIntent(mPowerKeyMode);
		}
			
	}

    private void CDRinterceptPowerKeyDown(KeyEvent event, boolean interactive) {
		
		/* 
		 * if mode = none set to 1
		 * set delay message for 2 sec
		 * */
            mPowerKeyMode = POWER_KEY_MODE_SHORT;
            Message msg = mHandler.obtainMessage(MSG_CDR_POWER_LONG_PRESS_SMODE);
            msg.setAsynchronous(true);
            mHandler.sendMessageDelayed(msg,CDR_POWERKEY_INTERVAL_SHORT);
		
    }
    
    private void CDRinterceptPowerKeyUp(KeyEvent event, boolean interactive, boolean canceled) {
		/*
         * cancel all handle for key mode 1 and 2 
         * check current key mode 
         * send intent about mode1 or mode2
         * set mode to none
         * */
		boolean notifyuser = false;
        switch (mPowerKeyMode){
			case POWER_KEY_MODE_SHORT:
			    mHandler.removeMessages(MSG_CDR_POWER_LONG_PRESS_SMODE);
			    notifyuser = true;
			    if(mPowerKeyWakeupSet && !interactive){
					wakeUpFromPowerKey(event.getDownTime());
				}
			    break;
			case POWER_KEY_MODE_NORMAL:
			    mHandler.removeMessages(MSG_CDR_POWER_LONG_PRESS_LMODE);
			    notifyuser = true;
		     	break;
		}
		if(notifyuser)
		...
		mPowerKeyMode = POWER_KEY_MODE_NONE;
    }
    
    private void sendPowerkeyIntent(int keymode) {  
		int ExtraValue = PKEY_TRIGGER_LONG;

		switch(keymode){
			case POWER_KEY_MODE_SHORT:
				ExtraValue = PKEY_TRIGGER_SHORT;
			break;
			case POWER_KEY_MODE_NORMAL:
				ExtraValue = PKEY_TRIGGER_NORMAL;
			break;
		}
		
		final Intent intent = new Intent(Intent.ACTION_POWERKEY_PRESS_TYPE);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY
                | Intent.FLAG_RECEIVER_REPLACE_PENDING);
                
        intent.putExtra(Intent.EXTRA_POWERKEY_PRESS_TYPE, ExtraValue);        
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                mContext.sendBroadcast(intent, null, UserHandle.USER_ALL);
            }
        });
	}

    private void sendKeyIntent(int keycode, int keymode) {
        int[] keystate = {keycode, keymode};
        Intent intent = new Intent(Intent.ACTION_KEY_PRESS_TYPE);
        intent.putExtra(Intent.EXTRA_KEY_PRESS_TYPE, keystate);
    ...
    private void powerLongPress() {
        ...
        case LONG_PRESS_POWER_GLOBAL_ACTIONS:
		    ...
		    sendPowerkeyIntent(POWER_KEY_MODE_NONE);
//            showGlobalActionsInternal();
            break;
    ...
    private void interceptScreenshotChord() {
        if (mScreenshotChordEnabled
                && mScreenshotChordDpadDownKeyTriggered && mScreenshotChordPowerKeyTriggered) {
            final long now = SystemClock.uptimeMillis();
            if (now <= mScreenshotChordDpadDownKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS
                    && now <= mScreenshotChordPowerKeyTime
                            + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS) {
                mScreenshotChordDpadDownKeyConsumed = true;
                cancelPendingPowerKeyAction();
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        mPowerKeySet = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_CDR6010_PowerKeySet);                
        mPowerKeyWakeupSet = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_CDR6010_PowerKeySet_Wakeup);					
        ...
        // register for set navigation bar item
        filter = new IntentFilter(INTENT_ACTION_SET_NAVIGATION_BAR_ITEM);
        context.registerReceiver(mNavigationNarItemReceiver, filter);
        ...
        mSystemGestures = new SystemGesturesPointerEventListener(context,
                new SystemGesturesPointerEventListener.Callbacks() {
                    ...
                    public void onSwipeFromBottom() {
                        ...
                        if (mNavigationBarPioneer != null && mNavigationBarOnBottom) {
                            requestTransientBars(mNavigationBarPioneer);
                        }
                    }
                    @Override
    ...
    public boolean checkShowToOwnerOnly(WindowManager.LayoutParams attrs) {
        ...
        switch (attrs.type) {
            ...
            case TYPE_NAVIGATION_BAR_PIONEER:
            case TYPE_PHONE:
            case TYPE_POINTER:
            case TYPE_PRIORITY_PHONE:
        ...
        switch (attrs.type) {
            ...
            case TYPE_NAVIGATION_BAR_PIONEER:
                mContext.enforceCallingOrSelfPermission(
                        android.Manifest.permission.STATUS_BAR_SERVICE,
                        "PhoneWindowManager");
                if (mNavigationBarPioneer != null) {
                    if (mNavigationBarPioneer.isAlive()) {
                        return WindowManagerGlobal.ADD_MULTIPLE_SINGLETON;
                    }
                }
                mNavigationBarPioneer = win;
                mNavigationBarPioneerController.setWindow(win);
                if (DEBUG_LAYOUT) Slog.i(TAG, "NAVIGATION BAR: " + mNavigationBarPioneer);
                break;
        }
        return WindowManagerGlobal.ADD_OKAY;
    }
    ...
    public void removeWindowLw(WindowState win) {
        ...
        if (mNavigationBarPioneer == win) {
            mNavigationBarPioneer = null;
            mNavigationBarPioneerController.setWindow(null);
        }
    }
    ...
    public int selectAnimationLw(WindowState win, int transit) {
        ...
        } else if (win == mNavigationBar || win == mNavigationBarPioneer) {
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (repeatCount == 0  && down && keyCode != mRecordedKey) {
            sendKeyIntent(keyCode, KEY_MODE_SHORT_PRESS);
            mRecordedKey = keyCode;
        } else if ((event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0) {
            sendKeyIntent(keyCode, KEY_MODE_LONG_PRESS);
        } else if (!down) {
            sendKeyIntent(keyCode, KEY_MODE_UP);
            mRecordedKey = 0;
        }
        ...
        if (mScreenshotChordEnabled && (flags & KeyEvent.FLAG_FALLBACK) == 0) {
            if (mScreenshotChordDpadDownKeyTriggered && !mScreenshotChordPowerKeyTriggered) {
                final long now = SystemClock.uptimeMillis();
                final long timeoutTime = mScreenshotChordDpadDownKeyTime
                        + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS;
                if (now < timeoutTime) {
            ...
            if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN
                    && mScreenshotChordDpadDownKeyConsumed) {
                if (!down) {
                    mScreenshotChordDpadDownKeyConsumed = false;
                }
                return -1;
            }
        }
    ...
    public int adjustSystemUiVisibilityLw(int visibility) {
        ...
        mNavigationBarPioneerController.adjustSystemUiVisibilityLw(mLastSystemUiFlags, visibility);
    ...
    public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth, int displayHeight,
        ...
        if (isDefaultDisplay) {
            ...
            if (mNavigationBar != null) {
                ...
                boolean hide_NavigationBarRight = (sysui & View.SYSTEM_UI_FLAG_HIDE_NAVIGATIONBAR_RIGHT)!=0;
                boolean hide_NavigationBarLeft = (sysui & View.SYSTEM_UI_FLAG_HIDE_NAVIGATIONBAR_LEFT)!=0;
                boolean hide_NavigationALL = (sysui & View.SYSTEM_UI_FLAG_HIDE_NAVIGATIONBAR)!=0;
                boolean hide_NavigationTransparent = (sysui & View.SYSTEM_UI_FLAG_NAVIGATIONBAR_TRANSPARENT)!=0;
                int left = 0;
                int top = 0;
                ...
                if (hide_NavigationBarRight) {
                    left = displayWidth - overscanRight
                            - mNavigationBarWidthForRotation[displayRotation];
                    top = displayHeight - overscanBottom
                                         - mNavigationBarHeightForRotation[displayRotation];
                    mTmpNavigationFrame.set(displayWidth, 0, displayWidth, 0);
                    mTmpNavigationFrameP.set(0, top, displayWidth, displayHeight);
                } else if (hide_NavigationBarLeft) {
                    left = displayWidth - overscanRight
                            - mNavigationBarWidthForRotation[displayRotation];
                    top = displayHeight;
                    mTmpNavigationFrame.set(left, 0, displayWidth, displayHeight);
                    mTmpNavigationFrameP.set(0, top, 0, displayHeight);
                } else if (hide_NavigationALL) {
                    left = displayWidth;
                    top = displayHeight;
                    mTmpNavigationFrame.set(displayWidth, 0, displayWidth, 0);
                    mTmpNavigationFrameP.set(0, top, 0, displayHeight);
                } else if (hide_NavigationTransparent) {
                    left = displayWidth;
                    top = displayHeight;
                    int leftBar = displayWidth - overscanRight
                            - mNavigationBarWidthForRotation[displayRotation];
                    int topBar = displayHeight - overscanBottom
                                         - mNavigationBarHeightForRotation[displayRotation];
                    mTmpNavigationFrame.set(leftBar, 0, displayWidth, displayHeight);
                    mTmpNavigationFrameP.set(0, topBar, leftBar, displayHeight);
                } else {
                    ...
                    top = displayHeight - overscanBottom
                                         - mNavigationBarHeightForRotation[displayRotation];
                    mTmpNavigationFrameP.set(0, top, left, displayHeight - overscanBottom);
                }
                mStableRight = mStableFullscreenRight = left;
                if (transientNavBarShowing) {
                    mNavigationBarController.setBarShowingLw(true);
                    mNavigationBarPioneerController.setBarShowingLw(true);
                } else if (navVisible) {
                    mNavigationBarController.setBarShowingLw(true);
                    mDockRight = left;
                    ...
                    mNavigationBarPioneerController.setBarShowingLw(false);
                }
                if (navVisible && !navTranslucent && !navAllowedHidden
                        && !mNavigationBar.isAnimatingLw()
                        && !mNavigationBarController.wasRecentlyTranslucent()
                        && !mNavigationBarPioneer.isAnimatingLw()
                        && !mNavigationBarPioneerController.wasRecentlyTranslucent()) {
                ...
                mNavigationBarPioneer.computeFrameLw(mTmpNavigationFrameP, mTmpNavigationFrameP,
                        mTmpNavigationFrameP, mTmpNavigationFrameP, mTmpNavigationFrameP, dcf,
                        mTmpNavigationFrameP, mTmpNavigationFrameP);
    ...
    public int getSystemDecorLayerLw() {
        ...
        if (mNavigationBarPioneer != null && mNavigationBarPioneer.isVisibleLw()) {
            return mNavigationBarPioneer.getSurfaceLayer();
        }
    ...
    public void layoutWindowLw(WindowState win, WindowState attached) {
        ...
        if ((win == mStatusBar && !canReceiveInput(win)) || win == mNavigationBar || win == mNavigationBarPioneer) {
            return;
        }
        ...
        final boolean hasNavBar = (isDefaultDisplay && mHasNavigationBar
                ...
                && mNavigationBarPioneer != null && mNavigationBarPioneer.isVisibleLw());
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                if (attrs.type == TYPE_STATUS_BAR_PANEL
                    ...
                    if (DEBUG_LAYOUT) Slog.v(TAG, String.format(
                        ...
                        || attrs.type == TYPE_NAVIGATION_BAR_PIONEER) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_DOWN:
                if (down) {
                    if (interactive && !mScreenshotChordDpadDownKeyTriggered
                            && (event.getFlags() & KeyEvent.FLAG_FALLBACK) == 0) {
                        mScreenshotChordDpadDownKeyTriggered = true;
                        mScreenshotChordDpadDownKeyTime = event.getDownTime();
                        mScreenshotChordDpadDownKeyConsumed = false;
                        cancelPendingPowerKeyAction();
                        interceptScreenshotChord();
                    }
                } else {
                    mScreenshotChordDpadDownKeyTriggered = false;
                    cancelPendingScreenshotChordAction();
                }
                break;
            // Add by SKYSOFT end
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                if(!mPowerKeySet){
					...
					if (down) {
                        CDRinterceptPowerKeyDown(event, interactive);
					}else{
						CDRinterceptPowerKeyUp(event, interactive,canceled);
					}
				}
                break;
            }
        ...
        if (mEnterPressed && !down) {
            enterDevelopMode(event);
        }
        return result;
    }

    /*
     * Enter ADB Mode method
     * 1. Hold Back key and press Volume Down -> Up -> Up -> Down
     * 2. interval less than 500ms
     */
    private void checkADBKey(KeyEvent event) {
        mCurrentTime = System.currentTimeMillis();
        mInterval = mCurrentTime - mLastTime;
        if (mInterval >= DEFAULT_INTERVAL_MS) {
            mStep = RESET;
        }
        if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_UP) {
            if (mStep == DOWN) {
                mStep = DOWN_UP;
            } else if (mStep == DOWN_UP) {
                mStep = DOWN_UP_UP;
            } else {
                mStep = RESET;
            }
    ...
    private void toggleADB() {
        IntentFilter ufilter = new IntentFilter(UsbManager.ACTION_USB_STATE);
        Intent usbStatus = mContext.registerReceiver(null, ufilter);
        boolean connected = usbStatus.getBooleanExtra(UsbManager.USB_CONNECTED, false);
        Log.d(TAG, "BD check USB connect status " + connected);
        ...
        if (0 != (Settings.Global.getInt(mContext.getContentResolver(),
            ...
            Log.d(TAG, "BD disabled ADB");
        } else {
            Settings.Global.putInt(mContext.getContentResolver(),
                    Settings.Global.ADB_ENABLED, 1);
            SystemProperties.set("persist.sys.usb.config", "mtp,adb");
            Log.d(TAG, "BD enabled ADB");
        }
    }

    /*
     * Enter Develop Mode method
     * 1. Press Volume Up -> Down -> Down -> Up
     * 2. interval less than 500ms
     */
    private void enterDevelopMode(KeyEvent event) {
        mDevelopCurrentTime = System.currentTimeMillis();
        mDevelopInterval = mDevelopCurrentTime - mDevelopLastTime;
        if (mDevelopInterval >= DEFAULT_INTERVAL_MS) {
            mDevelopStep = RESET;
        }
        if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_DOWN) {
            if (mDevelopStep == UP) {
                mDevelopStep = UP_DOWN;
            } else if (mDevelopStep == UP_DOWN) {
                mDevelopStep = UP_DOWN_DOWN;
            } else {
                mDevelopStep = RESET;
            }
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_UP) {
            if (mDevelopStep == UP_DOWN_DOWN) {
                showDeveolpModeDialog();
                mDevelopStep = RESET;
            } else {
                mDevelopStep = UP;
            }
        }
        mDevelopLastTime = mDevelopCurrentTime;
    }
    private void showDeveolpModeDialog() {
        Intent intent = new Intent(Intent.ACTION_SHOW_DEVELOPMODE);
        mContext.sendBroadcast(intent);
    }
    ...
    BroadcastReceiver mNavigationNarItemReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (INTENT_ACTION_SET_NAVIGATION_BAR_ITEM.equals(intent.getAction())) {
                final int[] items = intent.getIntArrayExtra(INTENT_ACTION_KEY);
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            try {
                                IStatusBarService statusbar = getStatusBarService();
                                    if (statusbar != null) {
                                        statusbar.setNavigationBarItem(items[0],items[1],items[2],items[3]);
                                    }
                                } catch (RemoteException e) {
                                    // re-acquire status bar service next time it is needed.
                                    mStatusBarService = null;
    ...
    private final Runnable mHiddenNavPanic = new Runnable() {
        ...
        public void run() {
            synchronized (mWindowManagerFuncs.getWindowManagerLock()) {
                ...
                mNavigationBarPioneerController.showTransient();
            }
        }
    };
    ...
    private void requestTransientBars(WindowState swipeTarget) {
        synchronized (mWindowManagerFuncs.getWindowManagerLock()) {
            ...
            boolean nb2 = mNavigationBarPioneerController.checkShowTransientBarLw();
            if (sb || nb || nb2) {
                ...
                if (!nb2 && swipeTarget == mNavigationBarPioneer) {
                    if (DEBUG) Slog.d(TAG, "Not showing transient bar, wrong swipe target");
                    return;
                }
                if (sb) mStatusBarController.showTransient();
                if (nb) mNavigationBarController.showTransient();
                if (nb2) mNavigationBarPioneerController.showTransient();
            ...
            mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            IStatusBarService statusbar = getStatusBarService();
                            if (statusbar != null) {
                                // set default navigation bar icon when screen update.
                                statusbar.setNavigationBarItem(p0136_btguid_icon02_5dis, p0148_btguid_icon06_5dis,
                                                                p0139_btguid_icon03_5dis, p0178_btguid_icon16_5dis);
    ...
    private int updateSystemBarsLw(WindowState win, int oldVis, int vis) {
        ...
        vis = mNavigationBarPioneerController.applyTranslucentFlagLw(transWin, vis, oldVis);
        ...
        boolean transientNavBarAllowedP =
                mNavigationBarPioneerController != null &&
                hideNavBarSysui && immersiveSticky;
        final long now = SystemClock.uptimeMillis();
        ...
        boolean denyTransientNavP = mNavigationBarPioneerController.isTransientShowRequested()
                && !transientNavBarAllowedP;
        ...
        vis = mNavigationBarPioneerController.updateVisibilityLw(transientNavBarAllowedP, oldVis, vis);
    ...
    private boolean isImmersiveMode(int vis) {
        ...
        return mNavigationBar != null && mNavigationBarPioneer != null
                && (vis & View.SYSTEM_UI_FLAG_HIDE_NAVIGATION) != 0
                && (vis & flags) != 0
    ...
    public void dump(String prefix, PrintWriter pw, String[] args) {
        ...
        if (mNavigationBarPioneer != null) {
            pw.print(prefix); pw.print("mNavigationBarPioneer=");
                    pw.println(mNavigationBarPioneer);
        }
        ...
        mNavigationBarPioneerController.dump(pw, prefix);
