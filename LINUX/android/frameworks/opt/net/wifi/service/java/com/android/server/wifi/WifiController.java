...
class WifiController extends StateMachine {
    ...
    private static final int WIFI_AP_DISABLED = 2;
    private static final int WIFI_AP_ENABLED = 3;
    // Add by Skysoft end
    ...
    WifiController(Context context, WifiServiceImpl service, Looper looper) {
        ...
        // Add by Skysoft start
        boolean isWifiApEnabled = mSettingsStore.isSoftapToggleEnabled();
        // Add by Skysoft end
            ...
            switch (msg.what) {
                ...
                case CMD_AIRPLANE_TOGGLED:
                    ...
                    // Add by Skysoft start
                        ...
                        int wifiApSavedState = Settings.Global.getInt(mContext.getContentResolver(),
                                    Settings.Global.WIFI_AP_SAVED_STATE, WIFI_AP_DISABLED);
                        Settings.Global.putInt(mContext.getContentResolver(),
                                    Settings.Global.WIFI_AP_SAVED_STATE, WIFI_AP_DISABLED);
                        if(wifiApSavedState == WIFI_AP_ENABLED) {
    ...
    class ApEnabledState extends State {
        @Override
        public boolean processMessage(Message msg) {
            if(DBG) logd("ApEnabledState preocess " + msg.what);
            switch (msg.what) {
                case CMD_AIRPLANE_TOGGLED:
                    if (mSettingsStore.isAirplaneModeOn()) {
                ...
                case CMD_WIFI_TOGGLED:
                    if (mSettingsStore.isWifiToggleEnabled()) {
                        Settings.Global.putInt(mContext.getContentResolver(),
                            Settings.Global.WIFI_AP_SAVED_STATE, WIFI_AP_ENABLED);
                        mWifiStateMachine.setHostApRunning(null, false);
                        deferMessage(msg);
                        transitionTo(mApStaDisabledState);
                    }
