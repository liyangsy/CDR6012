...
import static android.net.wifi.WifiManager.AP_WPS_STATUS_INACTIVE;
import static android.net.wifi.WifiManager.AP_WPS_STATUS_PROCESSING;
import static android.net.wifi.WifiManager.AP_WPS_STATUS_COMPLETED;
import static android.net.wifi.WifiManager.AP_WPS_STATUS_FAILED_TIMEOUT;
import static android.net.wifi.WifiManager.AP_WPS_STATUS_FAILED_OVERLAP;
/**
 * TODO:
...
public class WifiStateMachine extends StateMachine implements WifiNative.WifiPnoEventHandler,
    ...
    //Askey Added WPS Func for Pionner Req 082516 Scottj
    private static final String TETHER_WPS_STATE_CHANGED =
                "codeaurora.net.conn.TETHER_WPS_STATE_CHANGED";
    ...
    */
    ...
    /*Askey Added AP WPS Func for Pionner Req 082516 Scottj*/
    private static final int ASK_CMD_AP_WPS_ACTIVE                      = BASE + 166;
    private static final int ASK_CMD_AP_WPS_CANCEL                      = BASE + 167;
    private static final int ASK_AP_WPS_PROTECT                         = BASE + 168;
    private static final int ASK_AP_WPS_STATUS_RESET                    = BASE + 169;
    // Default WPS StsteMachine timeout @ 120Sec
    static final int WPS_GUARD_TIMER_MSEC = 120000;
    private int mApWpsRunningTimes = 0;
    private long mLastWPSPinCodeEvent = 0;
    //private int mApWpsStatus = 0;
    ...
    /* Waiting for SoftAP WPS to be completed*/
    ...
    /*Askey Added AP WPS Func for Pionner Req 082516 Scottj*/
    /**
     * One of  {@link WifiManager#AP_WPS_STATUS_INACTIVE},
     * {@link WifiManager#AP_WPS_STATUS_PROCESSING},
     * {@link WifiManager#AP_WPS_STATUS_COMPLETED},
     * {@link WifiManager#AP_WPS_STATUS_FAILED_TIMEOUT},
     * {@link WifiManager#AP_WPS_STATUS_FAILED_TIMEOUT}
     */
    private final AtomicInteger mApWpsStatus = new AtomicInteger(AP_WPS_STATUS_INACTIVE);
        ...
        addState(mDefaultState);
            ...
            addState(mSoftApStartedState, mDefaultState);
                ...
                addState(mTetheredState, mSoftApStartedState);
                    addState(mApWpsRunningState , mTetheredState);
     ...
     * TODO: doc
     */
    public int syncGetApWpsStatus() {
        return mApWpsStatus.get();
    }
...
//                        startScanNative(WifiNative.SCAN_WITHOUT_CONNECTION_SETUP, null);
    ...
    class SoftApStartedState extends State {
        ...
        // Add by Skysoft start
        private String readMacFromSysfs() {
            String sysfilePath = "/sys/class/net/wlan0/address";
            String macAddress = null;
            BufferedReader reader = null;
            String mac = null;

            try {
                reader = new BufferedReader(new FileReader(sysfilePath));
                mac = reader.readLine();
            } catch (FileNotFoundException e) {
                loge("Could not open " + sysfilePath + " to lookup mac address");
            } catch (IOException e) {
                loge("Could not read " + sysfilePath + " to lookup mac address");
            } finally {
                try {
                    if (reader != null) {
                        reader.close();
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case CMD_STOP_AP:
                    ...
                    break;
                /*Askey Added AP WPS Func for Pionner Req 082516 Scottj*/
                case ASK_CMD_AP_WPS_ACTIVE:
                    WpsResult wpsResult = new WpsResult();
                    String pinCode = (String)message.obj;
                    if (DBG) logd("TetheredState: WPS Act toogled: " + pinCode);
                    if (pinCode == null){
                        //PBC mode
                        try {
                            mNwService.startApWpsCommand(null);
                            wpsResult.status = Status.SUCCESS;
                        } catch (Exception e) {
                            Log.e(TAG, "Failed to start SoftAP WPS push button configuration");
                        ...
                        //PIN keypad mode
                        long now = System.currentTimeMillis();
                        Log.e(TAG, "Check pin result:" + pinCode);
                        if ((now - mLastWPSPinCodeEvent) > (WPS_GUARD_TIMER_MSEC/2)) {
                            try {
                                mNwService.startApWpsCommand(pinCode);
                                wpsResult.status= Status.SUCCESS;
                            } catch (Exception e) {
                                Log.e(TAG, "Failed to start SoftAP WPS pin method configuration");
                                wpsResult.status = Status.FAILURE;
                            }
                            mLastWPSPinCodeEvent = now;
                        } else {
                            //Cold down WPS PIN code mode for 60s
                            wpsResult.status = Status.FAILURE;
                        }
                        //break;
                    }
                    if (wpsResult.status == Status.SUCCESS) {
                        mApWpsStatus.set(AP_WPS_STATUS_PROCESSING);
                    ...
                    break;
                case ASK_AP_WPS_STATUS_RESET:
                    if (DBG) logd("arg1: " + message.arg1 + " RunningTimes: " + mApWpsRunningTimes);
                    if (message.arg1 == mApWpsRunningTimes) mApWpsStatus.set(AP_WPS_STATUS_INACTIVE);
                    break;
                case ASK_CMD_AP_WPS_CANCEL:
                    replyToMessage(message, message.what, FAILURE);
                    break;
                case WifiManager.CANCEL_WPS:
                case WifiMonitor.WPS_TIMEOUT_EVENT:
                    if (DBG) logd("TetheredState DBG MSG: " + message);
                    break;
                default:
                    return NOT_HANDLED;
            }
    ...
    class ApWpsRunningState extends State {
        @Override
        public void enter() {
            if (DBG) logd("Enter: ApWpsRunningState RunningTimes:" + mApWpsRunningTimes);
            sendMessageDelayed(obtainMessage(ASK_AP_WPS_PROTECT, mApWpsRunningTimes),
                    WPS_GUARD_TIMER_MSEC);
        }
        @Override
        public boolean processMessage(Message message) {
            switch(message.what) {
                case WifiMonitor.WPS_SUCCESS_EVENT:
                    Log.e(TAG, "ApWpsRunningState: WPS success ");
                    mApWpsStatus.set(AP_WPS_STATUS_COMPLETED);
                    transitionTo(mTetheredState);
                    break;
                case ASK_AP_WPS_PROTECT:
                    if (DBG) logd("arg1: " + message.arg1 + " RunningTimes: " + mApWpsRunningTimes);
                    if (message.arg1 != mApWpsRunningTimes) break;
                case WifiMonitor.WPS_TIMEOUT_EVENT:
                    Log.e(TAG, "ApWpsRunningState: WPS timeout ");
                    mApWpsStatus.set(AP_WPS_STATUS_FAILED_TIMEOUT);
                    transitionTo(mTetheredState);
                    break;
                case WifiMonitor.WPS_OVERLAP_EVENT:
                    Log.e(TAG, "ApWpsRunningState: WPS Overlap ");
                    mApWpsStatus.set(AP_WPS_STATUS_FAILED_OVERLAP);
                    transitionTo(mTetheredState);
                    break;
                case ASK_CMD_AP_WPS_CANCEL:
                    try {
                        mNwService.CancelApWpsCommand();
                        mApWpsStatus.set(AP_WPS_STATUS_INACTIVE);
                        replyToMessage(message, message.what, SUCCESS);
                    } catch (Exception e) {
                        mApWpsStatus.set(AP_WPS_STATUS_INACTIVE);
                        replyToMessage(message, message.what, FAILURE);
                ...
                case ASK_CMD_AP_WPS_ACTIVE:
                    replyToMessage(message, message.what, FAILURE);
                    if (DBG) logd("ApWpsRunningState DBG MSG: " + message);
                    break;
                case ASK_AP_WPS_STATUS_RESET:
                    break;
                case CMD_STOP_AP:
                    if (DBG) log("UnWPS and Untethering before stopping AP");
                    mApWpsStatus.set(AP_WPS_STATUS_INACTIVE);
                default:
                    return NOT_HANDLED;
            }
        ...
        public void exit() {
            mApWpsRunningTimes++;
            if (DBG) logd("Exit: ApWpsRunningState RunningTimes:" + mApWpsRunningTimes);
            sendMessageDelayed(obtainMessage(ASK_AP_WPS_STATUS_RESET, mApWpsRunningTimes), 30000);
        }
    }
    ...
    /*Askey Added AP WPS Func for Pionner Req 082516 Scottj*/
    private void WpsEventMonitor() {
        IntentFilter mFilter = new IntentFilter();
        mFilter.addAction(TETHER_WPS_STATE_CHANGED);
        BroadcastReceiver receiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                String message = intent.getStringExtra("event");
                if (DBG) logd("WpsEventMonitor onReceive: " + message);
                if (message == "WPS-PBC-ACTIVE") {
                    String obj = null;
                    sendMessage(obtainMessage(ASK_CMD_AP_WPS_ACTIVE, obj));
                } else if (message == "WPS-TIMEOUT" || message == "WPS-FAIL") {
                    sendMessage(obtainMessage(WifiMonitor.WPS_TIMEOUT_EVENT));
                } else if (message == "WPS-SUCCESS") {
                    sendMessage(obtainMessage(WifiMonitor.WPS_SUCCESS_EVENT));
                } else if (message == "WPS-OVERLAP-DETECTED") {
                    sendMessage(obtainMessage(WifiMonitor.WPS_OVERLAP_EVENT));
                }
            }
        };
    ...
    /*Askey Added AP WPS Func for Pionner Req 082516 Scottj*/
    public boolean syncStartApWps(AsyncChannel channel, String pinCode) {
        Message resultMsg = channel.sendMessageSynchronously(ASK_CMD_AP_WPS_ACTIVE, pinCode);
        boolean result = (resultMsg.arg1 != FAILURE);
        resultMsg.recycle();
        return result;
    }

    public boolean syncCancelApWps(AsyncChannel channel) {
        Message resultMsg = channel.sendMessageSynchronously(ASK_CMD_AP_WPS_CANCEL);
        boolean result = (resultMsg.arg1 != FAILURE);
        resultMsg.recycle();
        return result;
    }
