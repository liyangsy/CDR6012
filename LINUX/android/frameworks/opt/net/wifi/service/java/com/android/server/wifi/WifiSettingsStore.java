...
final class WifiSettingsStore {
    ...
    // Add by Skysoft start
    private int mPersistWifiApState = WIFI_AP_DISABLED;
    // Add by Skysoft end
    ...
    WifiSettingsStore(Context context) {
        ...
        // Add by Skysoft start
        mPersistWifiApState = getPersistedWifiApState();
        // Add by Skysoft end
    ...
    // Add by Skysoft start
    synchronized boolean isSoftapToggleEnabled() {
        if (mAirplaneModeOn) {
            return mPersistWifiApState == WIFI_ENABLED_AIRPLANE_OVERRIDE;
        } else {
            return mPersistWifiApState != WIFI_AP_DISABLED;
        }
    }
    // Add by Skysoft end
    ...
    void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        // Add by Skysoft start
        pw.println("mPersistWifiApState " + mPersistWifiApState);
        // Add by Skysoft end
    ...
    // Add by Skysoft start
    protected void persistWifiApState(int state) {
        final ContentResolver cr = mContext.getContentResolver();
    ...
    // Add by Skysoft start
    private boolean testAndClearWifiApSavedState() {
        final ContentResolver cr = mContext.getContentResolver();
        int wifiApSavedState = 0;
        try {
            wifiApSavedState = Settings.Global.getInt(cr, Settings.Global.WIFI_AP_SAVED_STATE);
            if(wifiApSavedState == 1)
                Settings.Global.putInt(cr, Settings.Global.WIFI_AP_SAVED_STATE, 0);
        } catch (Settings.SettingNotFoundException e) {
            ;
        }
        return (wifiApSavedState == 1);
    }
    // Add by Skysoft end
