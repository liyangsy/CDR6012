...
const nsecs_t POWER_BUTTON_LONG_PRESS_TIMEOUT = 10000 * 1000000LL; // 10sec

//if not power button
const nsecs_t BUTTON_LONG_PRESS_TIMEOUT = 2000 * 1000000LL; // 2sec
    ...
    if (! entry->dispatchInProgress) {
        if (entry->repeatCount == 0
            ...
            if (mKeyRepeatState.lastKeyEntry
                ...
                resetKeyRepeatLocked();
                if(entry->keyCode == KEYCODE_POWER) {
		  mKeyRepeatState.nextRepeatTime = entry->eventTime + POWER_BUTTON_LONG_PRESS_TIMEOUT;
		} else {
		    mKeyRepeatState.nextRepeatTime = entry->eventTime + BUTTON_LONG_PRESS_TIMEOUT;
		}
            }
