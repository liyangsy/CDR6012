...
static int clear_bootloader_message_block(const Volume* v);
...
int clear_bootloader_message() {
    ...
    if (strcmp(v->fs_type, "mtd") == 0) {
        ...
        return clear_bootloader_message_block(v);
    }
    LOGE("unknown misc partition fs_type \"%s\"\n", v->fs_type);
    return -1;
}
...
//fill 0 to misc partition
static int clear_bootloader_message_block(const Volume* v) {
    ui->Print("Wiping bootloader control block...\n");
    wait_for_device(v->blk_device);
    FILE* f = fopen(v->blk_device, "wb");
    if (f == NULL) {
        LOGE("Can't open %s\n(%s)\n", v->blk_device, strerror(errno));
